...
public class NPExoPlayer implements com.google.android.exoplayer.ExoPlayer.Listener, EventListener, HlsSampleSource.EventListener, BandwidthMeter.EventListener, MediaCodecVideoTrackRenderer.EventListener, MediaCodecAudioTrackRenderer.EventListener, StreamingDrmSessionManager.EventListener, DashChunkSource.EventListener, TextRenderer, MetadataTrackRenderer.MetadataRenderer<Map<String, Object>>, Provider {
    ...
    public interface RendererBuilder {
        void buildRenderers(NPExoPlayer nPExoPlayer);

        void cancel();
    }

    public NPExoPlayer(RendererBuilder rendererBuilder2) {
    ...
    /*public void onRenderers(TrackRenderer[] renderers, BandwidthMeter bandwidthMeter2) {
        ...
        CodecCounters codecCounters2 = this.videoRenderer instanceof MediaCodecTrackRenderer ? ((MediaCodecTrackRenderer) this.videoRenderer).codecCounters : renderers[1] instanceof MediaCodecTrackRenderer ? renderers[1].codecCounters : null;
        this.codecCounters = codecCounters2;
        this.bandwidthMeter = bandwidthMeter2;
        pushSurface(false);
        this.player.prepare(renderers);
        this.rendererBuildingState = 3;
    }*/void onRenderers(final TrackRenderer[] array, final BandwidthMeter bandwidthMeter) {
        final int n = 1;
        for (int i = 0; i < 4; ++i) {
            if (array[i] == null) {
                array[i] = new DummyTrackRenderer();
            }
        }
        this.videoRenderer = array[0];
        ...
        else if (array[n] instanceof MediaCodecTrackRenderer) {
            codecCounters = ((MediaCodecTrackRenderer)array[n]).codecCounters;
        }
        else {
            codecCounters = null;
        }
